...
namespace Kafiteria
{
    public partial class TJashte : Form
    {
        ...
        public TJashte(int IDA,string EmriAdm)
        {
            InitializeComponent();
        ...
        private void TJashte_Load(object sender, EventArgs e)
        {
            ShfaqTavolinat();
        }
        ...
        {
            conn.Open();
            string query = string.Format("SELECT * FROM Tavolinat where ID_Lokacioni=2");
        ...
        {
            conn.Open();
            sda = new SqlDataAdapter("INSERT INTO Tavolinat(X,Y,Hide,ID_Lokacioni) VALUES(1,1,1,2)", conn);
            sda.SelectCommand.ExecuteNonQuery();
            conn.Close();
            ParaqitTavolinat();
            panel1.Update();
